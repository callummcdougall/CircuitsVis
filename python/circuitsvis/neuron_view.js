...
requirejs(['jquery', 'd3'],
    function ($, d3) {
        ...
        function render() {
            ...
            var attnData = config.attention;
            var leftText = attnData.text;
            var rightText = attnData.text;
        ...
        function renderVisExpanded(svg, leftText, rightText, queries, keys) {
            ...
            var posDotProduct = posProduct + MATRIX_WIDTH + BOXHEIGHT / 2 + PADDING_WIDTH;
            var posSoftmax = posDotProduct + BOXHEIGHT + BOXHEIGHT / 2 + PADDING_WIDTH;
            var posRightText = posSoftmax + 2 * BOXHEIGHT + PADDING_WIDTH;

            svg = svg.append("g")
                .attr("id", "expanded")
                .attr("visibility", "hidden");

            renderHeadingsExpanded(svg, posQueries, posKeys, posProduct, posDotProduct, posSoftmax, posRightText);
            ...
            renderHorizLines(svg, "hlines2", posDotProduct - BOXHEIGHT / 2 - PADDING_WIDTH + 2, posDotProduct - BOXHEIGHT / 2);
            var dotProducts = new Array(rightText.length).fill(0);
            var softmaxed = new Array(rightText.length).fill(0);
            renderDotProducts(svg, dotProducts, posDotProduct);
            renderSoftmax(svg, softmaxed, posSoftmax + BOXHEIGHT / 2);
            renderText(svg, rightText, "rightText", posRightText, true);
            renderHorizLines(svg, "hlines3", posDotProduct + 1.5 * BOXHEIGHT + 2, posSoftmax);
        ...
        function renderHeadingsExpanded(svg, posQueries, posKeys, posProduct, posDotProduct, posSoftmax, posRightText) {
            ...
            var softmaxHeadingContainer = headingContainer.append("text")
                .attr("x", posSoftmax + 6)
                .attr("y", HEADING_HEIGHT - 12)
                .attr("height", BOXHEIGHT)
                .attr("width", MATRIX_WIDTH)
            ...
            softmaxHeadingContainer.append('tspan')
                .text('Probs')
                .style('font-size', HEADING_TEXT_SIZE + "px")
                .attr("y", HEADING_HEIGHT - 12);
        ...
        function renderHorizLines(svg, id, start_pos, end_pos) {
            var attnMatrix = config.attention.attn[config.layer][config.head];
        ...
        function renderTextQueryLines(svg, start_pos, end_pos) {
            var attnData = config.attention;
            var leftText = attnData.text; // Use for shape not values
            var linesContainer = svg.append("svg:g");
        ...
        function renderDotProducts(svg, dotProducts, leftPos) {
            var dotProductGroup = svg.append("svg:g")
                .attr("id", "dotproducts")
                .style("opacity", 0);
        
            dotProductGroup.selectAll("rect")
                .data(dotProducts)
                .enter()
                .append("rect")
                .classed('dotproduct', true)
                .attr("x", leftPos + 1 - BOXHEIGHT / 2)
                .attr("y", function (d, i) {
                    return i * BOXHEIGHT + HEADING_HEIGHT;
                })
            ...
            dotProductGroup.selectAll("text")
                .data(dotProducts)
                .enter()
                .append("text")
                .attr("x", leftPos + BOXHEIGHT / 2)  // adjust these to position your text correctly
                .text(function (d) {
                    return d.toFixed(2);
                })
                .style("text-anchor", "middle")
                .attr("y", function (d, i) {
                    return (i + 0.6) * BOXHEIGHT + HEADING_HEIGHT;
                });
        }
        
        function updateDotProducts(svg, dotProducts, dotProductMean, query_index) {
            ...
            vectorContainer.selectAll(".dotproduct")
                ...
                .style("fill-opacity", function (d) {
                    return Math.tanh(Math.abs(d - dotProductMean) / 4);
                })
                ...
                .style("stroke-opacity", function (d) {
                    ...
                    return Math.max(Math.tanh(Math.abs(d - dotProductMean) / 2), .35);
                })
                .attr("data-value", function (d) {
                    return d
                });
        ...
        // ! This is new! Can be replaced again with update-Softmax below (commented out)

        function renderSoftmax(svg, softmaxed, leftPos) {
            var softmaxGroup = svg.append("svg:g")
                .attr("id", "softmaxed")
                .style("opacity", 0);
        
            softmaxGroup.selectAll("rect")
                .data(softmaxed)
                .enter()
                .append("rect")
                .classed('dotproduct', true)
        ...
        function updateSoftmax(svg, softmaxed, query_index) {
            var vectorContainer = svg.select('#softmaxed').style("opacity", 1);
            vectorContainer.selectAll(".dotproduct")
                .data(softmaxed)
                .style("fill", getColor('pos'))
                .style("fill-opacity", function (d) {
                    return d;
                })
                .style("stroke", getColor('pos'))
                .style("stroke-opacity", function (d) {
                    // Set border to slightly higher opacity
                    return d;
                })
        ...
        function showComputation(svg, query_index) {
            ...
            // ! My addition: subtract mean of dot products (GPT4-assisted)

            var dotProductSums = 0;

            // Calculate dot products and their sum
            for (var i = 0; i < seq_len; i++) {
                ...
                dotProductSums += scaledDotProduct;
            }
            
            // Calculate the mean of the dot products
            var dotProductMean = dotProductSums / seq_len;
            
            // Subtract the mean from each individual dot product
            // for (var i = 0; i < seq_len; i++) {
            //     dotProducts[i] -= dotProductMean;
            // }
            
            updateVectors(svg, 'product', productVectors);
            updateDotProducts(svg, dotProducts, dotProductMean, query_index);
            updateSoftmax(svg, att, query_index);
        ...
        function hideComputation(svg) {
            ...
            svg.select("#softmaxed").style("opacity", 0);
        ...
        function initialize() {
            ...
            var attentionFilter = config.attention;
            ...
            config.initialTextLength = attentionFilter.text.length;
            config.expanded = true;
            ...
            if($(`#${config.rootDivId} #layer_and_head`).length) {
                const layerAndHeadSelect = $(`#${config.rootDivId} #layer_and_head`);
                // layerAndHeadSelect.empty();
                // for (var layer = 0; layer < config.nLayers; layer++) {
                //     for (var head = 0; head < config.nHeads; head++) {
                //         layerAndHeadSelect.append($("<option />").val(`(${layer},${head})`).text(`${layer}.${head}`));
                //     }
                // }
            
                layerAndHeadSelect.val(`(${config.layer},${config.head})`).change();
                layerAndHeadSelect.on('change', function (e) {
                    const [layer, head] = e.currentTarget.value.slice(1, -1).split(',').map(Number);
                    config.layer = layer;
                    config.head = head;
                    render();
                });
